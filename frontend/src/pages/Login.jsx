...
const useParams = () => ({ role: "staff" }); // change to "owner" to preview owner mode
const api = {
  get: async () => ({ data: [{ id: 1, name: "Alappuzha Main" }, { id: 2, name: "Cherthala" }, { id: 3, name: "Kayamkulam" }] }),
  post: async () => ({ data: { token: "eyJ.eyJyb2xlIjoic3RhZmYifQ.sig", calling_name: "Rajan" } }),
};
...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const callingName = res.data?.calling_name || res.data?.staff_name || res.data?.name || username || "User";
  ...
  const css = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .lr-input-wrap {
      ...
      overflow: hidden; /* keeps toggle inside border-radius */
    }
    ...
    .lr-input {
      ...
      font-family: 'DM Sans', sans-serif;
      color: #1A0808;
      /* Prevent iOS zoom (must be ≥16px effective) */
      font-size: max(16px, 15px);
    }
    .lr-input::placeholder { color: #C4AFAF; }

    /* Password toggle button — sits INSIDE the wrapper */
    .lr-pwd-toggle {
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    /* Tablet (≤900px) — narrow left panel */
    @media (max-width: 900px) {
      .lr-left { width: 360px; padding: 36px 28px 28px; }
      .lr-headline { font-size: 28px; }
    }

    /* Large phone landscape / small tablet (≤700px) — stack vertically */
    @media (max-width: 700px) {
      ...
      .lr-left {
        ...
        /* Compact on mobile: hide bulky hero text, keep brand + feat */
      }
      /* Hide hero headline/sub on mobile — keep features visible */
      .lr-hero { padding: 16px 0 8px; }
      .lr-headline { font-size: 24px; }
    ...
    @media (max-width: 480px) {
      ...
      /* Only show brand + features on phone, hide full hero text */
      ...
      .lr-input, .lr-select { font-size: 16px; /* hard floor for iOS */ }
    }

    /* Small Android (≤360px) — really tight */
    @media (max-width: 360px) {
      ...
      /* Stack features into 1 col — already column, just tighten */
          ...
          <div className="lr-form-box">
            ...
            <div className="lr-fields">
              ...
              <div className="lr-field">
                ...
                {/*
                  FIX: The password input and toggle button are siblings
                  inside ONE lr-input-wrap div. This ensures:
                  - Toggle is always vertically centred (flexbox)
                  - Border-radius is shared via overflow:hidden
                  - No gap or misalignment between input and button
                  - Focus ring wraps both elements together
                */}
                <div className="lr-input-wrap">
                  <input
                    id="password"
              ...
              {/* Submit */}
              <div>
                <div className="lr-divider">or continue with password</div>
                <button
                  className="lr-btn"
                  onClick={handleLogin}
